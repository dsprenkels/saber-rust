...
impl Poly {
    ...
    pub(crate) fn from_bytes_13bit(bytes: &[u8]) -> Self {
        ...
        for (bs, cs) in bytes.chunks_exact(13).zip(poly.coeffs.chunks_exact_mut(8)) {
            ...
            cs[1] = (U16::from(U8::from(bs[1])) >> 5) | (U16::from(U8::from(bs[2])) << 3) | (U16::from(U8::from(bs[3])) << 11);
            cs[2] = (U16::from(U8::from(bs[3])) >> 2) | (U16::from(U8::from(bs[4])) << 6);
            cs[3] = (U16::from(U8::from(bs[4])) >> 7) | (U16::from(U8::from(bs[5])) << 1) | (U16::from(U8::from(bs[6])) << 9);
            cs[4] = (U16::from(U8::from(bs[6])) >> 4) | (U16::from(U8::from(bs[7])) << 4) | (U16::from(U8::from(bs[8])) << 12);
            cs[5] = (U16::from(U8::from(bs[8])) >> 1) | (U16::from(U8::from(bs[9])) << 7);
            cs[6] = (U16::from(U8::from(bs[9])) >> 6) | (U16::from(U8::from(bs[10])) << 2) | (U16::from(U8::from(bs[11])) << 10);
    ...
    pub(crate) fn from_bytes_10bit(bytes: &[u8]) -> Self {
        ...
        for (bs, cs) in bytes.chunks_exact(5).zip(poly.coeffs.chunks_exact_mut(4)) {
            cs[0] = U16::from(U8::from(bs[0])) & 0xFF.into() | (U16::from(U8::from(bs[1]) & 0x03.into()) << 8);
            cs[1] = ((U16::from(U8::from(bs[1])) >> 2) & 0x3F.into()) | (U16::from(U8::from(bs[2]) & 0x0F.into()) << 6);
            cs[2] = ((U16::from(U8::from(bs[2])) >> 4) & 0x0F.into()) | (U16::from(U8::from(bs[3]) & 0x3F.into()) << 4);
            cs[3] = ((U16::from(U8::from(bs[3])) >> 6) & 0x03.into()) | (U16::from(U8::from(bs[4])) << 2);
        }
        poly
    }
    ...
    pub(crate) fn from_bytes_6bit(bytes: &[u8]) -> Self {
        ...
        for (bs, cs) in bytes.chunks_exact(3).zip(poly.coeffs.chunks_exact_mut(4)) {
            ...
            cs[1] = U16::from((U8::from(bs[0]) >> 6) & 0x03.into()) | U16::from((U8::from(bs[1]) & 0x0F.into()) << 2);
            cs[2] = U16::from((U8::from(bs[1]) >> 4) & 0x0F.into()) | U16::from((U8::from(bs[2]) & 0x03.into()) << 4);
    ...
    pub(crate) fn from_bytes_3bit(bytes: &[u8]) -> Self {
        ...
        for (bs, cs) in bytes.chunks_exact(3).zip(poly.coeffs.chunks_exact_mut(8)) {
            ...
            cs[2] = U16::from((U8::from(bs[0]) >> 6) & 0x03.into()) | U16::from((U8::from(bs[1]) & 0x01.into()) << 2);
            cs[3] = U16::from((U8::from(bs[1]) >> 1) & 0x07.into());
            cs[4] = U16::from((U8::from(bs[1]) >> 4) & 0x07.into());
            cs[5] = U16::from((U8::from(bs[1]) >> 7) & 0x01.into()) | U16::from((U8::from(bs[2]) & 0x03.into()) << 1);
    ...
    pub(crate) fn from_msg(msg: &[u8]) -> Self {
        ...
        for (b, coeffs_chunk) in msg.iter().zip(m_poly.coeffs.chunks_exact_mut(8)) {
            for (idx, coeff) in coeffs_chunk.iter_mut().enumerate() {
                *coeff = U16::from((U8::from(*b) >> (idx as u32)) & 0x01.into()) << MSG2POL_CONST.into();
            }
        }
        m_poly
    }
